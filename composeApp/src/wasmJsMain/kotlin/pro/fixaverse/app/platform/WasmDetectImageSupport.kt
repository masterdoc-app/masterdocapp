...
@JsName("fixaverseCompressDetectImage")
internal external fun fixaverseCompressDetectImage(
    input: JsAny,
    onReady: (JsAny?) -> Unit,
)

internal fun handleCameraPayload(data: JsAny, onResult: (PickedImage?) -> Unit) {
    fixaverseCompressDetectImage(data) { compressed ->
...
internal fun deliverPickedFile(
    ...
    println("[fixaverse detect] wasm ready ${bytes.size} bytes from $resolvedName")
