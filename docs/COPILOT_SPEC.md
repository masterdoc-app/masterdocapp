...
> **В реестре AI-агентов ТОиР** ([TOIR_AI_SYSTEM_DESIGN §3](../../masterdoc/TOIR_AI_SYSTEM_DESIGN.md)) этот контур называется агент **Copilot** (бывш. «Наставник»): read-only ответы инженеру по документации актива с цитатами. Backend: отдельный **`copilot-service`** (`POST /ai/copilot/*`), без общего ai-gateway. Соседние сервисы: `technologist-service` (**admin** загружает доки и оборудование → карточки автоматически), `intake-service`, `closeout-service` (агент Репортер). Шильдик/QR — только идентификация актива **инженером в поле**, не вход Технолога.
...
Целевой backend (ТОиР): чат Copilot идёт в **`copilot-service`** через API Gateway (`POST /ai/copilot/*`), не в общий ai-gateway. Поиск по документам — через тот же контур + `search-service` (Onyx). См. [TOIR_AI_SYSTEM_DESIGN §4](../../masterdoc/TOIR_AI_SYSTEM_DESIGN.md).
