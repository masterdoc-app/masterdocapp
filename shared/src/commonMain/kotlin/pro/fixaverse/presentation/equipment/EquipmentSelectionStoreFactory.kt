package pro.fixaverse.presentation.equipment
        ...
        scope.launch {
            ...
            repository.detectAssistant(
                ...
                .onSuccess { detectedName ->
                    ...
                    if (assistant != null) {
                        println("[fixaverse detect] matched assistant id=${assistant.id} name=${assistant.name}")
                        dispatch(Msg.Selected(assistant))
                    } else {
                        println("[fixaverse detect] no match for detectedName=$detectedName")
                        dispatch(
                ...
                .onFailure { error ->
                    dispatch(Msg.SetDetecting(false))
                    println("[fixaverse detect] failed: ${error.message}")
                    dispatch(
                        Msg.SetDetectError(
                            error.message?.takeIf { it.isNotBlank() }
