...
class DefaultRootComponentBackTest {
    ...
    @Test
    fun onBack_fromChatGuide_returnsToChatDescribe() {
        val root = createRoot()
        root.chat.equipmentStore.accept(
            EquipmentSelectionStore.Intent.Select(Assistant(id = 1, name = "Холодильники")),
        )
        ...
        assertIs<FlowChild.ChatGuide>(root.stack.value.active.instance)
    ...
    private fun createRoot(): DefaultRootComponent {
        ...
        val root = DefaultRootComponent(
            ...
            summaryStoreFactory = SummaryStoreFactory(storeFactory),
        )
        lifecycle.start()
        lifecycle.resume()
        return root
